...
#define mpz_set_bignum(var_mpz,var_bignum) do{ \
  VALUE var_bignum_str = rb_funcall(var_bignum, rb_intern("to_s"), 0); \
  mpz_set_str(var_mpz, StringValuePtr(var_bignum_str), 0); \
} while(0)
    
#define mpz_temp_alloc(var) { var=malloc(sizeof(MP_INT)); }
#define mpz_temp_init(var) { mpz_temp_alloc(var); mpz_init(var); }
#define mpz_temp_from_bignum(var,var_bignum)  do{ \
  VALUE var_bignum_str = rb_funcall(var_bignum, rb_intern("to_s"), 0); \
  mpz_temp_alloc(var); \
  mpz_init_set_str(var, StringValuePtr(var_bignum_str), 0); \
  } while(0)
...
#endif
#define prec_max(prec,var) { prec = max(mpf_get_prec(var), (int)prec); }

/* 1.8 / 1.9 compat */
#ifndef RFLOAT_VALUE
#define RFLOAT_VALUE(var) (RFLOAT(var)->value)
#endif
